...
if __name__ == '__main__':
    ...
    elif sys.argv[1] == '-n':  # python3 cowsay.py -n COW MESSAGE
        name = sys.argv[2]
        MESSAGE = ''
    ...
    elif sys.argv[1] == '-f':  # python3 cowsay.py -f COW MESSAGE
        name = sys.argv[2]
        MESSAGE = ''
        for i in range(3, len(sys.argv)):
            MESSAGE += sys.argv[i] + ' '

        path = os.path.join('cows', name + '.cow')
        if os.path.isfile(path):
            fcow = FileCow(name, path)
            print(MESSAGE)
            print(fcow.get_image())
        else:
            print(f'Could not find {name} cow!')


    else:  # python3 cowsay.py MESSAGE
        MESSAGE = ''
